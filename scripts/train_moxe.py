...
def loss_fn(
    ...
    layers_output = output.layers_output

    z_loss = layers_output.z_loss.mean()
    load_balancing_loss = layers_output.load_balancing_loss.mean()

    # d-loss
    d_loss: jax.Array = jax.lax.cond(
        d_loss_coef > 0.0,
        lambda: layers_output.conditioned_output.d_loss.mean(),
    ...
    group_loss: jax.Array = jax.lax.cond(
        group_loss_coef > 0.0,
        lambda: layers_output.conditioned_output.group_loss.mean(),
        lambda: jnp.zeros((), dtype=_dtype),
    )
